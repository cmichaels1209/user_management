...
from app.schemas.user_schemas import (
    ...
    UserResponse, UserUpdate, UpgradeProfessionalRequest
)
...
async def update_own_profile(
    ...
    return UserResponse.model_construct(**updated_user.to_dict(request))
...
async def get_user(
    ...
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    return UserResponse.model_construct(**user.to_dict(request))
...
async def upgrade_professional_status(
    user_id: UUID,
    upgrade_data: UpgradeProfessionalRequest,
    request: Request,
    db: AsyncSession = Depends(get_db),
    current_user: dict = Depends(require_role(["ADMIN", "MANAGER"]))
):
    updated_user = await UserService.update(db, user_id, {"is_professional": upgrade_data.is_professional})
    if not updated_user:
        raise HTTPException(status_code=404, detail="User not found")
    return UserResponse.model_construct(**updated_user.to_dict(request))
...
async def create_user(
    ...
    return UserResponse.model_construct(**created_user.to_dict(request))
...
async def login(
    ...
    access_token = create_access_token(
        data={"sub": user.email, "role": str(user.role.name) if user.role else "ANONYMOUS"},
